...
export default function JournalManager() {
    ...
    const ToolbarButton = ({ onClick, isActive, children, title }: any) => (
        <button type="button" onClick={onClick} className={isActive ? "is-active" : ""} title={title}>
            {children}
        </button>
    );
            ...
            <div className="relative z-10 mx-auto max-w-6xl px-6">
                <div className="mb-12">
                    <h1 className="text-4xl font-light tracking-wide">
                        <span className="bg-gradient-to-r from-white to-[#D4AF37] bg-clip-text text-transparent">Journal Manager</span>
                    </h1>
                    <p className="mt-2 text-sm text-[#a3a3a3]">{editingId ? "Edit existing article" : "Create a new article"}</p>
                </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input className="w-full rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-white focus:border-[#D4AF37]/40 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40" value={titleEn} onChange={(e) => handleTitleEn(e.target.value)} />
                            </div>
                            <div>
                                <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-[#D4AF37]">Title (FA) *</label>
                                <input className="w-full rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-white focus:border-[#D4AF37]/40 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40" value={titleFa} onChange={(e) => setTitleFa(e.target.value)} dir="rtl" />
                            </div>
                        </div>

                        <div>
                            <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-[#D4AF37]">Slug * (URL-friendly)</label>
                            <input className="w-full rounded-xl border border-white/10 bg-white/[0.03] p-3 font-mono text-sm text-white focus:border-[#D4AF37]/40 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40" value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""))} />
                        </div>

                        <div className="grid gap-6 md:grid-cols-2">
                            <div>
                                <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-[#D4AF37]">Excerpt (EN)</label>
                                <textarea className="h-24 w-full resize-none rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-white focus:border-[#D4AF37]/40 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40" value={excerptEn} onChange={(e) => setExcerptEn(e.target.value)} />
                            </div>
                            <div>
                                <label className="mb-2 block text-xs uppercase tracking-[0.2em] text-[#D4AF37]">Excerpt (FA)</label>
                                <textarea className="h-24 w-full resize-none rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-white focus:border-[#D4AF37]/40 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]/40" value={excerptFa} onChange={(e) => setExcerptFa(e.target.value)} dir="rtl" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div dir={activeTab === "fa" ? "rtl" : "ltr"}>
                                ...
                                <div className="tiptap-wrapper">
                                    ...
                                    <div className="tiptap-editor" onWheel={(e) => e.stopPropagation()}>
